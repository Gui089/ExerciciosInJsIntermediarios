...
setTimeout(() => {
  console.log('Se passrarm 3 segundos ');
}, 3000);
...
const filterNumbers = numbers.filter(getLessThanFive);
const lesserThanFive = filterNumbers;
...
const myFun = (pram1, param2, param3) => {
  const ifSomeParamUndefined = [pram1, param2, param3].includes(undefined);
  if(ifSomeParamUndefined) {
...
booksBox.addBooks = booksQtd => {
  if(booksBox.booksIn === booksBox.spaces) {
    return 'Caixa esta cheia';
  }

  if(booksBox.booksIn + booksQtd > booksBox.spaces) {
    const avalibeSpaces = booksBox.spaces - booksBox.booksIn;
    const pluralOrSingular = avalibeSpaces === 1 ? 'Livro' : 'Livros';
    return `Só cabem mais ${avalibeSpaces} ${pluralOrSingular}`;
  }

  booksBox.booksIn += booksQtd;
